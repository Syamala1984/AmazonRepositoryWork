...
public class NotesDaoImpl implements NotesDao {
	...
	public void saveOrUpdate(Notes notes) {
		if (notes.getId() != null && notes.getId() > 0) {
			...
			jdbcTemplate.update(sql, notes.getName(), notes.getType(),
					notes.getUpdatedDate(), notes.getLocation(),
			...
			jdbcTemplate.update(sql, notes.getName(), notes.getContent(), notes
					.getType(), notes.getCreatedDate(), notes.getFolder()
					.getId(), notes.getUser().getId(), notes.getStatus(), notes
					.getLocation());
