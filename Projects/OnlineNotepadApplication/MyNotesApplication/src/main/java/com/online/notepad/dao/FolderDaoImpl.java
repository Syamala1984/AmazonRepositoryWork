...
public class FolderDaoImpl implements FolderDao {
	...
	public void save(Folder folder) {
		// insert
		String sql = "INSERT INTO folder (name, location, created_date,status)"
				+ " VALUES (?,?,?,?)";
		logger.info("\n===SQL  ::  " + sql + "\n===" + folder.getName() + "\n"
				+ folder.getLocation() + "\n" + folder.getCreatedDate() + "\n");
		jdbcTemplate.update(sql, folder.getName(), folder.getLocation(),
				folder.getCreatedDate(), "Active");
	...
	public Folder getById(int id) {
		...
		return jdbcTemplate.query(sql, new ResultSetExtractor<Folder>() {
			...
			@Override
			public Folder extractData(ResultSet rs) throws SQLException,
					DataAccessException {
				if (rs.next()) {
	...
	public List<Folder> list() {
		...
		List<Folder> listFolder = jdbcTemplate.query(sql,
